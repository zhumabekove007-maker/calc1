...
(function () {
  ...
  function formulaBoxHtml(formula) {
    return (
      '<div class="formula-box">' +
        ...
        '<div class="formula-tex">\\[' + formula.tex + '\\]</div>' +
      '</div>'
    );
  }

  function workedExampleHtml(example) {
    var steps = example.steps.map(function (s) { return '<li>' + s + '</li>'; }).join('');
    return (
  ...
  function mistakesCalloutHtml(mistakes) {
    var items = mistakes.map(function (m) { return '<li>' + m + '</li>'; }).join('');
    return (
  ...
  function subsectionTopicHtml(chapter, sub, opts) {
    ...
    return (
      '<section class="subsection-block" id="' + sub.id + '" data-sub-id="' + sub.id + '" tabindex="-1">' +
        ...
        '</div>' +
        '<p class="core-idea">' + sub.coreIdea + '</p>' +
        formulas +
